...
- 你可以从CoreELEC的电源菜单中点击Reboot to EmuELEC来重启到EmuELEC，或者，你可以在SSH中输入reboot\_to\_emuelec.sh（前者会调用的脚本）来设置下一次重启至EmuELEC而不实际重启
- 首次以EmuELEC开机时，请参考官方EmuELEC的设置流程，由于小米盒子3/3c的emmc过慢，且初次开机需要向emmc写入400M左右数据，首次开机会较慢。建议使用USB手柄完成首次设置，你可以在之后更换到蓝牙手柄。
- 你可以从EmuELEC的快速访问菜单中点击REBOOT TO COERELEC来重启到CoreELEC，或者，你可以在SSH中输入reboot\_to\_coreelec.sh（前者会调用的脚本）来设置下一次重启至CoreELEC而不重启
...
对docker容器与镜像一些常规操作命令备忘
```
 docker ps -a       ##显示所有的容器id与状态，包括未运行的
    docker rm 容器id   ##删除容器，容器id前4位即可

    docker images      ##显示所有的镜像id与状态，包括未运行的
    docker rmi 镜像id  ##删除本地镜像，镜像id前4位即可
...
### alist
```
docker pull xhofe/alist:v3.12.2

docker run -d --restart=always -v /storage/docker/alist:/opt/alist/data -p 5244:5244 --name="alist" xhofe/alist:v3.12.2
...
3、创建一个docker的虚拟网络
docker network create -d macvlan --subnet=192.168.5.0/24 --gateway=192.168.5.1 -o parent=wlan0 macnet

4、拉取镜像到本地
docker pull unifreq/openwrt-aarch64:r20.05.20

5、安装镜像 
docker run -d --restart always --name openwrt --network macnet --ip 192.168.5.10 --privileged unifreq/openwrt-aarch64:r20.05.20 /sbin/init
...
CoreELEC已经集成了安装脚本，SSH登录你的CoreELEC，键入如下代码，即可完成安装
...
`https://blog.csdn.net/sinat_36544290/article/details/104690855
