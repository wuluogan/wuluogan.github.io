...
var onloadStartGame = function () {
    ...
    try {
        ...
        function StartGame() {
            ...
            Hero["Name"] = chairmanHome.name;
            Hero["HP"] = Math.round(chairmanHome.hp, 1);
            Hero["ATK"] = Math.round(chairmanHome.atk, 1);
            Hero["DEF"] = Math.round(chairmanHome.def, 1);
            Hero["Gold"] = Math.round(chairmanHome.gold, 1);
            Hero["Exp"] = Math.round(chairmanHome.exp, 1);
            Hero["YellowKey"] = 0;
            Hero["BlueKey"] = 0;
            Hero["RedKey"] = 0;
            Hero["IronKey"] = 0;
            ...
            function CreateMapControl(MapBg, MapEvent, MapFg, SystemUI, DataUpdate, ResData, MapsData, IconsData, ItemsData) {
                ...
                m.DrawGoFloorPanel = function (Type) {
                    ...
                    if (!Flag.ShowGoFloor) {
                        ...
                        m.Data.fillText(((ControllerMode == 0) ? ("点击按键的上或下选择要跳跃到的楼层后，") : ("按 ↑ 或 ↓ 键选择要跳跃到的楼层后，")), 176, 325);
...
var saveGameButtonClick = function () {
    var chairmanHome = getHomeChairman();
    chairmanHome.exp = Hero["Exp"];
    saveGame(1);
    //autoSaveGame();
    return;
}

var autoSaveGame = function () {
    var chairmanHome = getHomeChairman();
    chairmanHome.exp = Hero["Exp"];
    saveGame(0);
    setTimeout("autoSaveGame();",60000);
